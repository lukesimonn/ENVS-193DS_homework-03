...
There are two appropriate tests for testing the strength of the relationship between temperature and giant kelp frond elongation rate (correlation). These two are the Pearson's correlation, which assumes normally distributed continuous data, and Spearman's rank correlation, which does not require normality but does require continuous data and a monotonic distribution. Pearsons's correlation is the correct test to choose when there are no outliers in the data and the distribution is clearly normal, in all other cases Spearman's is more appropriate.
...
ggplot(data = kelp,
  ...
  geom_point(color = "tan") + # changing point color
...
For my affective data visualization I want to create a network map of all of my data as nodes. These nodes would vary in shape and color which would each correspond to some detail from the data set. My current thought process, though this will likely change, is that color would operate on a gradient for walk time and node shape would correspond to arrival time (number of sides = minutes after 7:50). I would also include some signifier for if I took a stop and connect each node by location. With the path of nodes representing chronlogical progression. Node location relative to each other is based on temperature (x-axis) and wake up time (y-axis) and node border color signifies location.

**NOTE: I chose arrival time due to spread being lower than departure time (less shapes needed), departure time is easily calculated with color and shape**
...
![](images/affectivedraft.png)

## d.

This piece is a network map displaying my daily walk time to my 8 am class, heavily influenced by the creation of real network maps I have encountered and the shape and color work of Posavec and Lupi. This was created on the free and open source network mapping platform draw.io. Paths are created to represent which of the two buildings I was going to (in chronological order), colors follow rainbow order (red-purple) for walk time (red = 12, purple = 21) and the number of sides of each shape represents how many minutes after 7:50 I arrived, horizontal location is by daily temperature, while vertical is by wake-up time.

## e.

[Slideshow link](https://docs.google.com/presentation/d/10PTqVswPO0zJS4wEpbgHM269NQ7UjAgUEE3OfZESkVc/edit?slide=id.g3eb33348aa7_0_21#slide=id.g3eb33348aa7_0_21)

# Problem 4. Statistical critique

## a. 

The author is trying to answer the question of whether or not competition between herbaceous and woody plants depends on the availability of anabolic nutrients relative to catabolic nutrients. To do so, the selected test was the Kruskal-Wallis test where the response variable is either tree height or tree abundance and the predictor is treatment type. They are doing this so that they can analyze if there is a difference in tree height/abundance across treatment type.

![](images/plot.png)

## b.

This plot is fairly visually clear and concise. The axes both make sense, though perhaps under the x-axis they could label with "Treatment" but this is fairly clear in context of the paper. Median values for each treatment type are clearly visibile and easy to relate to the y-axis.

## c.

I think that the authors handle visual clutter very well. This is going to be a cramped figure no matter what due to the sheer number of treatment types. I think the line widths for each element are thoughtfully chosen to make them distinct yet not too close together. The figure can appear to be a lot at first but it is quite clear once some time is spent with it. The data:ink ratio isn't great, but there is not much that can be done to fix this due to the large spread of the treatments.

## d.

The change I would make would be to replace the SP and AS with their full name SP -> superphosphate and AS -> ammonium sulphate. I also think including treatment on the x-axis would be very helpful for making the figure stand alone. To help with clutter caused by expanding the names I think you could do away with the repeat of the treatment types for the second plot and utilize one concise x-axis. If the authors wanted to split this into two figures they could also include the underlying data, but I think this would be too crowded in its current side-by-side configuration.